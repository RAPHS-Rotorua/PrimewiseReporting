...
CREATE Procedure [dbo].[rpt_SmokingRegister_MissedOpportunity_WHHC_Test] @PracticeIDs VARCHAR(MAX)
as
SET NOCOUNT ON	
...
@AuthorisedPracticeIDList AS PID
	...
	where 
	h.LastEvent >= dateadd(day,-15,Cast(GetDate() as Date))
	and h.CessLastScreen < dateadd(day,-15,Cast(GetDate() as Date))