CREATE Proc [dbo].[Flu_Vac_PriorityList_65Plus_Linc_Declined]  @PracticeIDs VARCHAR(MAX)
as
SET NOCOUNT ON;
...
p.ProviderCode, p.EnrolledInPMS,
...
-
	...
	End >=65,'65 Plus','Less than 65') as '65 Plus', f.Status_Desc, f.IMMDate as 'Date Assessed'
from 
...
join Patient as p on p.PracticeID = PID.PracticeID
	...
	join FluVaccinations as f on f.NHI = p.NHI
where (EnrolledInPMS = 'enrolled' or Funding = 'Capitated')
and p.PracticeID <> 0 and f.Outcome = 'Declined'
) as z
