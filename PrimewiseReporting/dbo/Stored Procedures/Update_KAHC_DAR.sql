Create proc [dbo].[Update_KAHC_DAR]
as
...
Select NHI, iif([DAR Last Completed] >= @Start,1,0) as 'Numerator', [DAR Last Completed],
 ...
 from StaticData.dbo.Stage_DAR_Reporting_KAHC_20240122 --Select  from StaticData.dbo.Stage_DAR_Reporting_WHHC_20240620
)
update i
	set i.In_Numerator = p.Numerator, 
		i.Dar_Status = iif(i.Dar_Status <> 'Deceased',p.Dar_Status_New,i.Dar_Status),
			 i.Dar_Status_sort = iif(i.Dar_Status <> 'Deceased', p.Dar_Status_Sort_New, i.Dar_Status_Sort),
				i.[DAR Last Completed] = p.[DAR Last Completed]
