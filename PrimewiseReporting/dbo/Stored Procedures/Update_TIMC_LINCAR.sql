Create proc [dbo].[Update_TIMC_LINCAR]
as
...
from StaticData.dbo.Stage_DAR_Reporting_TIMC_20250106 --change this:  Select * from StaticData.dbo.Stage_DAR_Reporting_TIMC_20250106
where [DAR Last Completed] is not null
union all
Select NHI, Last_Completed, LTC_Status
from StaticData.dbo.Stage_LINC_Reporting_TIMC_20250106  --change this:  Select * from StaticData.dbo.Stage_LINC_Reporting_TIMC_20250106
where Last_Completed is not null
) as z) as x
where row_num = 1)
--Select * from LINCAR_OLD_Profile
	update i
	...
	from Stage_LINC_Reporting as i  
		...
		where i.PracticeID = 28 and i.In_Numerator = 0 and p.Numerator = 1;  --change this:  PracticeID