...
***/
CREATE Procedure [dbo].[Dar_PracticePerformance_Summary_Temp] @PracticeIDs VARCHAR(MAX)
as
SET NOCOUNT ON	
...
from Dar_Reporting_TempTest as h1 where h1.Ethnicity1Description = 'NZ Maori' and h1.PracticeID = h.PracticeID) as Performance_Maori,
(Sum(h.In_Denominator) * 0.8) - Sum(h.In_Numerator) as No_Required_To_Target,
(Select (Sum(h2.In_Denominator) * 0.8) - Sum(h2.In_Numerator) 
